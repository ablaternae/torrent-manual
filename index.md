...
>Bencode
>
>Бинарный формат описания, используемый системой распространения файлов BitTorrent для хранения и передачи свободно структурированных данных.
>
...
Начальный символ `i` и конечный `е` &mdash; начальный и конечный разделители, соответственно. Целому числу не может предшествовать префикс, состоящий из нулей, например, `i03e`. Вместе с тем, `i0e` является является корректной записью нуля.
...
{: .highlight }
**ПРИМЕЧАНИЕ**: Максимальное количество бит целого числа не описывается, но для рассмотрения больших файлов, таких как торренты более 4-х гигабайт, необходимо знаковое 64-битное целое поле.
