...
Этот документ относится к первой версии 1.0 спецификации протокола BitTorrent. На данный момент он отражает файловую структуру торентов, протоколу однорангового обмена (peer wire) и спецификации HTTP/HTTPS-трекеров. По мере выхода новых версий этих протоколов, они должны быть описаны на отдельных страницах, но **не здесь**.
...
{: .highlight }
**ПРИМЕЧАНИЕ**: Максимальное количество бит целого числа не описывается, но для рассмотрения больших файлов, таких как торенты размером более 4-х гигабайт, необходимо знаковое целое 64-битное поле.
...
* `info`: Словарь описывает файл(ы) торента. Есть две возможных формы: одна для случая с однофайловым торентом, без описания структуры каталогов; другая для многофайлового торента (подробности ниже)

* `announce`: URL трекера для публикации торента (строка)
...
* `creation date *`: Дата создания торента в стандартном формате UNIX-времени (unixtimestamp, целое число секунд, прошедших с 01.01.1970 00:00:00 UTC)
...
* `piece length`: Размер каждой _части_ (_piece_) в байтах (целое).

* `pieces`: Строка, составленная объединением всех 20-байтовых значений SHA1-хэшей каждой _части_, одна хэш-строка на каждую часть (байтовая строка, **не** urlencoded).

* `private *`: Поле является целым числом. Если оно установлено в значение `1`, клиент ДОЛЖЕН обозначить свое присутствие, чтобы получать другие одноранговые узлы (список _пиров_) ТОЛЬКО через трекеры, явно прописанные в файле метаданных. Если поле установлено в значение `0` или отсутствует, клиент может получать список пиров другими средствами, например, через обмен пирами [PEX](https://ru.wikipedia.org/wiki/PEX) (Peering EXchange), или технологию [DHT](https://ru.wikipedia.org/wiki/DHT) (Distributed Hash Table). Таким образом, название поле «частный» можно понимать как «без внешних узлов».
...
name: Имя файла, который содержит торент. Носит рекомендательный характер. (строка)

length: Размер файла в байтах (целое).

md5sum: (опциональный) 32-символьная шестнадцатеричная строка соответствующая MD5-сумме файла. Она не используется в BitTorrent, но записывается в торент некоторыми программами для лучшей совместимости.

Info в много-файловом режиме

В случае много-файлового режима, хэш-таблица info дополняется следующими ключами: 

name: Имя корневой директории, которую содержит торент. Носит рекомендательный характер. (строка) 
...
Ключ 'piece length' устанавливает номинальный размер куска, который, как правило, кратен 2-м. Размер куска обычно выбирается исходя из общего количества файловых данных в торенте, учитывая то, что слишком большой размер куска неэффективен, а слишком маленький даёт в результате большой торент файл. При выборе наименьшего размера куска руководствуйтесь здравым смыслом. Желательно не делать торент-файл размером больше, чем 50-75КБ (чтобы облегчить его загрузку на сервер). Однако, сейчас, когда размеры хостинга и ширина каналов не сильно ограничиваются, лучший размер куска для более эффективной раздачи файлов равен 512КБ или меньше (по крайней мере для торентов примерно до 8-10ГБ), даже если это приведёт к увеличению торент-файла. Часто используемые размеры — 256КБ, 512КБ и 1МБ. Каждый кусок равен выбранному размеру, за исключением последнего, размер которого может быть меньше. Количество кусков вычисляется делением общего размера файлов торента на размер куска и округляется в большую сторону. Для вычисления границ кусков в много-файловом торенте, файловые данные рассматриваются как один большой непрерывный поток, составленный объединением содержимого всех файлов в порядке их следования в списке 'files'. Число кусков и их границы определяются таким же образом, как и в случае одного файла. Куски могут перекрывать границы файлов (т.е. начало куска может находится в конце предыдущего файла, а конец — в начале следующего).
...
Трекер — это HTTP/HTTPS сервис, который отвечает на HTTP GET запросы. Запросы содержат в себя метрику от клиентов, помогающая трекеру вести статистику для торента. В ответах содержится список пиров, чтобы клиент мог участвовать в раздаче. Базовый URL запроса состоит из 'announce URL', который определён в файле мета-данных (торент-файл), и параметров, которые добавляются к этому URL'у при помощи стандартного CGI-метода (т.е. добавление '?' после announce-URL, за которым следует последовательности 'параметр=значение', разделённые символом '&').
...
complete: Число пиров, имеющих все файлы торента. Их называют сидерами (целое)

incomplete: Число пиров, не имеющих все файлы торента. Их называют личерами (целое)
...
Большинство трекеров поддерживают другую форму запроса, которая используется для получения информации по определенному торенту (или всех торентов), которыми управляет трекер. Вместо неудобного парсинга страницы со статистикой, клиент может отправить такой запрос, и трекер ответит так называемой scrape-страницей.
...
files: Хэш-таблица, содержащая одну пару ключ/значение для каждого торента, по которому есть статистика. Если задан валидный параметр 'info_hash', то таблица содержит одну пару ключ/значение. Каждый ключ — это 20-байтовое бинарное значение 'info_hash'. Значение ключа — это еще одна хэш-таблица:

complete: Число пиров, имеющих все файлы торента (сидеры) (целое)

downloaded: Общее количество завершенных закачек, зарегистрированных трекером (регистрируются при событии 'event=complete', то есть когда клиент закончил скачивание торента)

incomplete: Число пиров, не имеющих все файлы торента (личеры) (целое) 

name: (опциональный) Внутреннее имя торента, указанное в ключе 'name' раздела 'info' торент-файла
...
Где .................... — это 20-байтовое значение параметра 'info_hash' для торента, со статистикой: 5 сидеров, 10 личеров и 50 завершенных закачек.
...
Протокол связи между пирами (далее, просто peer-протокол) облегчает обмен кусками (pieces), перечисленных в торент-файле.

Обратите внимание, что при описании peer-протокола в оригинальной спецификации используется термин "кусок", однако это не тот "кусок", который используется при описании торент-файла. Поэтому, в этой спецификации будет использоваться термин "блок" для обозначения данных, которыми обмениваются пиры по сети.
...
info_hash: 20-байтовый SHA1-хэш ключа 'info' торент-файла. Это тот же 'info_hash', который передается в запросах трекеру.
...
Инициатор соединения немедленно посылает handshake-сообщение. Адресат может отложить ответ инициатору, если он обслуживает несколько торентов одновременно (торенты однозначно идентифицируются по их 'info_hash'). Несмотря на это, адресат должен ответить сразу, как только получит значение поля 'info_hash' в handshake-сообщении. Трекерная функция NAT-проверки не посылает поле 'peer_id' при рукопожатии.

Клиент должен оборвать соединение, если получил handshake-сообщение с 'info_hash' торента, который он не обслуживает.
...
Свойство супер сид (super-seed) в и над S-5.5 является новым алгоритмом сидирования, спроектированным для помощи инициатору торента с ограниченной пропускной способностью  "накачивать" большой торент, уменьшая количество объема данных, необходимых для выгрузки (upload) и для порождения новых сидов в торенте.
...
Этот метод привел к намного более высокой эффективности сидирования, посредством принуждения пиров в получении только редчайших данных, это и  сокращение избыточного количества посылаемых данных, и ограничение объема данных, посылаемых пирам, которые не способствуют распространению этих данных в рое. До этого, сид должен был выгрузить от 150% до 200% от общего размера торента перед тем как другие клиенты становятся сидами. Однако, большой торент , раздаваемый в режиме 'супер сид' в состоянии набрать первых сидов уже после  выгрузки 105% данных. Это намного эффективнее, чем при использовании стандартного режима.
...
Возможность раздавать торент через веб-сервер обычно называется WebSeeding. Она позволяет HTTP-серверу выступать в роли пира в BitTorrent сети.
