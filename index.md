...
Начальный символ `i` и конечный `е` - начальный и конечный разделители соответственно. Вы можете кодировать отрицательные числа, такие как `i-3e`. Целому числу не может предшествовать префикс, состоящий из нулей, например, `i04e`. Вместе с тем, `i0e` является является корректной записью нуля.
